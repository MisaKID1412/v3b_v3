...
def process_face(
    ...
    if args.footprint_min_area > 0.0:
        footprint_penalty = (
            np.clip(
                (args.footprint_min_area - footprint_map) / max(args.footprint_min_area, 1e-6),
                0.0,
                1.0,
            )
            * 0.55
    ...
    if args.surface_distance_tol > 0.0:
        surface_penalty = (
            np.clip(
                surface_distance_map / max(float(args.surface_distance_tol), 1e-6),
                0.0,
                1.0,
            )
            * 0.60
    ...
    Image.fromarray(np.clip(footprint_map / max(args.footprint_min_area, 1e-6) * 255.0, 0, 255).astype(np.uint8)).save(dbg_dir / f"{face}_footprint_area.png")
...
def main() -> int:
    ...
    (args.out_dir / "photo_source_manifest.json").write_text(json.dumps(out_manifest, indent=2, ensure_ascii=False), encoding="utf-8")
